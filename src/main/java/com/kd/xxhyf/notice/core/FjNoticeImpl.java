...
public class FjNoticeImpl implements Notice{
	...
	public void run(String receiveString) {//线程启动方法
		...
		for(Object value : ids){//循环set
		    ...
		    for (int i = 0; i < dataList.size(); i++) {//进行单个数据处理
				...
				String sql = "SELECT * FROM "+FJPERFIX+"SYS_NOTICE WHERE NOTICE_DESC='"+jsonObject.toString()+"' AND TABLE_ID='"+type+"'";//需要执行的SQL
				...
				if(list.size()==0){//只有在数据库中不存在时再执行
					...
					case "1"://新增 add
						sql = "INSERT INTO "+FJPERFIX+"SYS_NOTICE (NOTICE_DESC,TABLE_ID,START_TIME,OPT,DATA_SOURCE) VALUES('"+jsonObject.toString()+"','"+type+"','"+sdf.format(new Date())+"','add','"+data_source+"');";
						connection.execute(sql);
						break;
					case "2"://更新 update
						sql = "INSERT INTO "+FJPERFIX+"SYS_NOTICE (NOTICE_DESC,TABLE_ID,START_TIME,OPT,DATA_SOURCE) VALUES('"+jsonObject.toString()+"','"+type+"','"+sdf.format(new Date())+"','update','"+data_source+"');";
						connection.execute(sql);
						break;
					case "3"://退运 back
						sql = "INSERT INTO "+FJPERFIX+"SYS_NOTICE (NOTICE_DESC,TABLE_ID,START_TIME,OPT,DATA_SOURCE) VALUES('"+jsonObject.toString()+"','"+type+"','"+sdf.format(new Date())+"','back','"+data_source+"');";
						connection.execute(sql);
						break;
					case "4"://通知 notice
						sql = "INSERT INTO "+FJPERFIX+"SYS_NOTICE (NOTICE_DESC,TABLE_ID,START_TIME,OPT,DATA_SOURCE) VALUES('"+jsonObject.toString()+"','"+type+"','"+sdf.format(new Date())+"','notice','"+data_source+"');";
						connection.execute(sql);
						break;
					default:
						break;
